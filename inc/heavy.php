...
function check_heavy_request() {
  ...
  if (config("heavy_requests_seconds") >= 0) {
    ...
    if ($heavy = $q->fetch()) {
      ...
      if (strtotime($heavy['last_request']) > strtotime("-" . config("heavy_requests_seconds") . " seconds")) {
        throw new BlockedException("You are making too many requests at once: please wait at least " . config("heavy_requests_seconds") . " seconds.");
        // throw new BlockedException(t("You are making too many requests at once: please wait at least :seconds.", array(':seconds' => plural("second", config("heavy_requests_seconds")))));
      } else {
        // update database
